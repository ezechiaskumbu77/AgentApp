...
class AddPPCProductSales extends StatefulWidget {
  ...
  AddPPCProductSales({Key key, this.shop, this.product }) : super(key: key);
  @override
  _AddPPCProductSalesState createState() => _AddPPCProductSalesState();
}

class _AddPPCProductSalesState extends State<AddPPCProductSales>
  ...
  _pressCreateAccountButton() async{
    ...
    if( editcontrolNumber.text.isEmpty  || editControlprice.text.isEmpty){
     ...
     // print("name : ${editcontrolName.text} et devise ${_devise}");
      ...
      bool isCreate = await  _productSalesService.create( _saleStockModel);
      if(isCreate ) {
    ...
    return ListView(
      ...
      children: <Widget>[
        ...
        Container(
          margin: EdgeInsets.only(left: 15, right: 20),
          alignment: Alignment.centerLeft,
          child: Text("Stock disponible : ${widget.product.inventorOnHand}",                        // "Create an Account!"
              style: TextStyle(fontSize: 18, color: Colors.white, fontStyle: FontStyle.italic)
          ),
        ),
        SizedBox(height: 5,),
