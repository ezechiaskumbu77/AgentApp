...
class SupplyStockType extends StatefulWidget {

ShopModel shop ;
ProductStockModel product ;

 SupplyStockType({this.shop, this.product});

  @override
  _SupplyStockTypeState  createState() =>  _SupplyStockTypeState();
}

class _SupplyStockTypeState extends State<SupplyStockType> {
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Center(child: Text("Approvisionnement",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 20),)),
        ...
        if (btnState == ButtonState.Idle) {
          ...
          Timer(const Duration(milliseconds: 1000), () {
              ...
              Navigator.of(context).push(
                  MaterialPageRoute(
                      builder: (context) => AddStock(shop: widget.shop,product: widget.product,)
                  )
              );
      ...
      onTap: (startLoading, stopLoading, btnState) {
        if (btnState == ButtonState.Idle) {
          ...
          Timer(const Duration(milliseconds: 1000), () {
            ...
            Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) => SupplyList(shop: shop,productStockModel: widget.product,)
                )
            );
